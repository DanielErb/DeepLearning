    ...
    def buildNeuralNetwork(self):
        ...
        for i in range(1, self.numLayers + 1):  # runs from 1 to layers
            if i == self.numLayers:
                ...
                self.layers.append(Softmax(W = w, b = b , networkType = self.networkType))
                ...
                self.weights.append(w)
                self.biases.append(b)
                self.layers.append(LayerFunc(W = w, b = b, Activation = self.activationFuncName , networkType = self.networkType))
            else:
                num_weights = random.randint(50, 100)
